...
while ADU_cur_Hmax>ADU_thres*prominence
    ...
    idx_map=zeros(size(pixel_map_uncrop));
...
% speedup        lin_idx_sub(lin_idx_sub==(Hbox_size*2+1)^2*Hbox_size_axi+(Hbox_size*2+1)*Hbox_size+Hbox_size+1)=[];
% speedup        [sub_row,sub_col,sub_page]=ind2sub(size(cur_sub_vol),lin_idx_sub);
% speedup        vec_dist=sqrt((sub_row-Hbox_size-1).^2+(sub_col-Hbox_size-1).^2+((sub_page-Hbox_size_axi-1)*aspect_ratio).^2);
% speedup        vec_dist_std=sqrt(sum(vec_dist.^2)/length(vec_dist));
%         mean_vec_dist=mean(vec_dist);
%         median_vec_dist=median(vec_dist);
% speedup        num_vec_dist=length(vec_dist);
        %         idx=find(vec_dist<Hbox_size);
        %         vec_dist_psf=vec_dist(idx);
        %(median_vec_dist>para.sim_median_dist*0.9)&&(median_vec_dist<para.sim_median_dist*1.1)&&(mean_vec_dist>para.sim_mean_dist*0.9)&&(mean_vec_dist<para.sim_mean_dist*1.1)
        num_vec_dist=length(lin_idx_sub)-1;
