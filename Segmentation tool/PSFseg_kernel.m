...
Hbox_size=para.Hbox_size_search; 
...
% while ADU_cur_Hmax>ADU_thres*prominence
    ...
    while sum(peak_value_crop)>0
        ...
        num_vec_dist=length(idx_above_fwhm);
...
dist_thres=15;
if para.FM
    emitter_temp=emitter_fine;
    emitter_fine=[];
    emitter_temp_uni=emitter_temp;
    emitter_temp_uni(:,3)=emitter_temp_uni(:,3)*stepsize/1000/pixelsize;
    while ~isempty(emitter_temp)
        emitter_fine=[emitter_fine;emitter_temp(1,:)];
        emitter_temp(1,:)=[];
        cur_emitter=emitter_temp_uni(1,:);
        emitter_temp_uni(1,:)=[];
        dist_arr=sqrt(sum((cur_emitter-emitter_temp_uni).^2,2));
        emitter_temp(dist_arr<dist_thres,:)=[];
        emitter_temp_uni(dist_arr<dist_thres,:)=[];
    end
end
...
IMG_MIP=IMG_MIP(:,:,height_seg+1:slice_num-height_seg);%IMG_MIP(:,:,height_seg:slice_num-height_seg+1);
