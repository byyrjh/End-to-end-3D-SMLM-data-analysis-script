...
load('map_ptr_t_FM_m0.mat');
load('map_ptr_x_FM_m0.mat');
load('map_ptr_y_FM_m0.mat');
load('map_ptr_z_FM_m0.mat');
load('camera1_cali.mat');
load('seg_data_FM_m0.mat');
num_seg = length(map_ptr_x_FM);
inten_tr = zeros(5,num_seg);
for i = 1:num_seg
    temp_seg = seg_data_FM(:,:,(i-1)*5+1:i*5);
    temp_os = offset(map_ptr_x_FM(i)-9:map_ptr_x_FM(i)+9,map_ptr_y_FM(i)-9:map_ptr_y_FM(i)+9);
    temp_gain = gain(map_ptr_x_FM(i)-9:map_ptr_x_FM(i)+9,map_ptr_y_FM(i)-9:map_ptr_y_FM(i)+9);
...
bin_edge_rsq = linspace(-400,400,100);
figure
histogram(RMSE_vec,bin_edge_rmse)
xlabel('Normalized RMSE')
figure
histogram(mu_vec,bin_edge_rsq)
xlabel('R-squared')
