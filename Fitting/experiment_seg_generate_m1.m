...
SlicesPerStack_indentifier_start='hyperstackand';
SlicesPerStack_indentifier_end='frames/stack';
...
pt_idf_SPS_start=strfind(prt_content,convertCharsToStrings(SlicesPerStack_indentifier_start));
pt_idf_SPS_end=strfind(prt_content,convertCharsToStrings(SlicesPerStack_indentifier_end));
ROIinfo=prt_content{1}(pt_idf_start(1)+10:pt_idf_end(1)-1);
SlicesPerStack=str2num(prt_content{1}(pt_idf_SPS_start(1)+13:pt_idf_SPS_end(1)-1));
...
load(strcat(data_path,'PSFdata_FM_m1.mat'));
...
for i=1:num_valid_FM
    ...
    temp=seg_data_FM(:,:,(cur_idx-1)*SlicesPerSeg+1:cur_idx*SlicesPerSeg);
    temp_reord=zeros(size(temp));
    for j=1:SlicesPerSeg
        temp_reord(:,:,j)=temp(:,:,SlicesPerSeg+1-j);
    end
    seg_data(:,:,(i-1)*SlicesPerSeg+1:i*SlicesPerSeg)=temp_reord;
end
...
map_ptr_z_FM=single(SlicesPerStack+1-valid_FM(:,3)/stepsize*pixelsize);
map_ptr_t_FM=single(valid_FM(:,5));
seg_data_FM=single(seg_data);
save(strcat(data_path,'segment_data\seg_data_FM_m1.mat'),'seg_data_FM');
save(strcat(data_path,'segment_data\map_ptr_x_FM_m1.mat'),'map_ptr_x_FM');
save(strcat(data_path,'segment_data\map_ptr_y_FM_m1.mat'),'map_ptr_y_FM');
save(strcat(data_path,'segment_data\map_ptr_t_FM_m1.mat'),'map_ptr_t_FM');
save(strcat(data_path,'segment_data\map_ptr_z_FM_m1.mat'),'map_ptr_z_FM');
...
load(strcat(data_path,'PSFdata_SM_m1.mat'));
data.data(spike_idx) = [];
overall_stack_num=size(data.data,1);
SlicesPerSeg=size(data.data{1,1}.VolumeSet{1,1},3);
...
for stack_idx=1:overall_stack_num
    ...
    for i=1:num_emitter(stack_idx,1)
        temp=data.data{stack_idx,1}.VolumeSet{i,1};
        temp_reord=zeros(size(temp));
        for j=1:SlicesPerSeg
            temp_reord(:,:,j)=temp(:,:,SlicesPerSeg+1-j);
        end
        seg_data_SM(:,:,sum(num_emitter(1:stack_idx-1,1))*SlicesPerSeg+(i-1)*SlicesPerSeg+1:sum(num_emitter(1:stack_idx-1,1))*SlicesPerSeg+i*SlicesPerSeg)=temp_reord;
    end
end
map_ptr_x_SM=single(map_ptr_x_SM);
map_ptr_y_SM=single(map_ptr_y_SM);
map_ptr_z_SM=single(SlicesPerStack+1-map_ptr_z_SM);
map_ptr_t_SM=single(map_ptr_t_SM);
seg_data_SM=single(seg_data_SM);
save(strcat(data_path,'segment_data\seg_data_SM_m1.mat'),'seg_data_SM', '-v7.3');
save(strcat(data_path,'segment_data\map_ptr_x_SM_m1.mat'),'map_ptr_x_SM');
save(strcat(data_path,'segment_data\map_ptr_y_SM_m1.mat'),'map_ptr_y_SM');
save(strcat(data_path,'segment_data\map_ptr_z_SM_m1.mat'),'map_ptr_z_SM');
save(strcat(data_path,'segment_data\map_ptr_t_SM_m1.mat'),'map_ptr_t_SM');
...
save(strcat(data_path,'segment_data\fitting_info_m1.mat'),'num_FM','num_SM','slice_num_FM','slice_num_SM','num_vol','camsize_x','camsize_y','stationary_pos','vol_per_hyper');